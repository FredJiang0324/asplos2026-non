                              ...
                              const uint32_t Lf)
{
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024)
    {
        ...
        diskann::IndexWriteParameters paras = diskann::IndexWriteParametersBuilder(L, R)
                                                  ...
                                                  .build();
        using TagT = uint32_t;
        std::string temFile = "temReorderedData.bin";
        ///TODO: for billion-scale version -- need to write the pageVectors into files --- because it may be too large to load into memory
        std::string pageCenterFile = "pageCenterData.bin";
        std::vector<T> pageVectors;//store 2d Vector as 1D
        //using Block Scope so that _index is destructed as soon as the block is exited.
        {
...
//newly added codes
//TODO: actually, we dont need to create and use Vaman_reader here. we just need to use _graph_store through _index to get the info we want
            uint32_t width_u32;
            ...
            //update the calculation -- so that we can consider the overhead of storing compressed data
            uint64_t npts, max_nbrs_space, max_node_len, nnodes_per_sector;
            npts = (uint64_t)base_num;
            max_nbrs_space = (uint64_t)((width_u32 + 1) * sizeof(uint32_t)); //nbrIDs + num of nbrs
            max_node_len = (uint64_t)(base_dim * sizeof(T));//size of will return in unit of Byte
            ...
            diskann::cout << "nnodes_per_sector: " << nnodes_per_sector << std::endl;
             ///TODO: need to update the nodes capacity with the consideration of compressed page center data overhead
            _index.mergeNodesIntoPage((uint32_t)base_num, (uint32_t)base_dim, nnodes_per_sector, temFile, pageVectors);//pass this pageCenterFile intead of pageVectors
        }

        size_t num_page_pts = pageVectors.size() / base_dim;

        std::cout << "Size of pageVectors: " << num_page_pts << std::endl;

        ///TODO: write pageVectors into the file pageCenterFile--- for future version --- integrate this function into _index.mergeNodesIntoPage
        //pageCenterFile
        write(pageVectors, pageCenterFile);

        std::string compressedPageCenterFile = "compressedPageCenterData.bin";
...
//newly added codes
        //create new index with new num_pts
        //defaults::NUM_FROZEN_POINTS_STATIC is 0
        diskann::Index<T, TagT, LabelT> _index(compareMetric, base_dim, num_page_pts,
                                                std::make_shared<diskann::IndexWriteParameters>(paras), nullptr,
                                                defaults::NUM_FROZEN_POINTS_STATIC, false, false, false,
                                                build_pq_bytes > 0, build_pq_bytes, use_opq, use_filters);
                                         
        std::vector<TagT> tags = std::vector<TagT>(); 
        ///TODO: for billion-scale version -- need to read the pageVectors from files instead of pass it through container
        _index.build(pageVectors.data(), num_page_pts, tags);
        //std::string mem_page_graph_index_path =  "page_graph_mem.index";
        //_index.save(mem_page_graph_index_path.c_str()); //this may not be needed at all; since we still use this _index to create disk layout

        //create disk layout
        std::string final_outputFile = "finalPageGraphIndex.index";
        ///TODO: add the pageCenter compressed data into disk page data
        _index.create_disk_layout(num_page_pts, temFile, final_outputFile);//because we need the _index to compute the distance and sort and get closest node to each page and vice versa
        //in searching, we need to new data stucture for storing info of each page
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
        ...
        {
            pFlashIndex->cached_beam_search(tuning_sample + (i * tuning_sample_aligned_dim), 1, L, warmupValuesOfReturnNodes,
...
//TODO: what is the usage of the sample points?
...
//TODO: remove temReorderedData, page_graph_index_mem
    if (use_disk_pq)
