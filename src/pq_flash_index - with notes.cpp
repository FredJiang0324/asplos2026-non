...
//        pq_query_scratch->initialize(this->_data_dim, aligned_query_T);
    ...
    //Prefetch data into level 2 and higher caches.
...
//TODO: what is _pq_table -- FixedChunkPQTable
...
//    uint8_t *pq_coord_scratch = pq_query_scratch->aligned_pq_coord_scratch;
    ...
    //uint8_t *data -- all different types of pq indices. for example 4 chunks, [0. 256, 122, 1], [1, 2, 4, 256] --- using index to access this ID set 
    //memcpy(out + i * ndims, data + ids[i] * ndims, ndims * sizeof(uint8_t)); 1 int corresponds to n_chuck * uint8 -- i.e., PQ index, namely a set of ids of each closest centroids in each sub-vector/chuck
    //pq_coord_scratch is holder buffer for pq indices of the node, initialy contains nothing
auto compute_dists = [this, pq_coord_scratch, pq_dists](const uint32_t *ids, const uint64_t n_ids,
                                                        float *dists_out) {
    diskann::aggregate_coords(ids, n_ids, this->data, this->_n_chunks, pq_coord_scratch); //store the PQ ID in pq_coord_scratch
    ...
    NeighborPriorityQueue &retset = query_scratch->retset;//it has data set which is a vector of Neighbor
    retset.reserve(l_search);
    std::vector<Neighbor> &full_retset = query_scratch->full_retset;
//read and understand till here
    ...
    if (!use_filter)
    {
        for (uint64_t cur_m = 0; cur_m < _num_medoids; cur_m++)
        {
//TODO: query_float is still from pq_query_scratch
// _dist_cmp compare T while _dist_cmp_float compares float
...
//TODO: update --- dist_scratch is from pq_query_scratch
//why/if need/how to precompute query/dists for non-pq cases
//1 represent n_ids/n_pts
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        while (retset.has_unexpanded_node() && frontier.size() < beam_width && num_seen < beam_width)
        {
            ...
            num_seen++;
            auto iter = _nhood_cache.find(nbr.id); //tsl::robin_map<uint32_t, std::pair<uint32_t, uint32_t *>> _nhood_cache;
            //the key is id, the first element of the pair value is nnbrs, and the second element of the pair value is uint32_t *node_nbrs
        ...
        if (!frontier.empty())
        {
            ...
            for (uint64_t i = 0; i < frontier.size(); i++)
            {
                ...
                sector_scratch_idx++;
                frontier_nhoods.push_back(fnhood);//data -- the whole sector is read into this fnhood's second field -- that is why it is a char*
                //constructs an element in place at the end of the container. -- AlignedRead
        ...
        {
            auto global_cache_iter = _coord_cache.find(cached_nhood.first); // tsl::robin_map<uint32_t, T *> _coord_cache; the key of the map is the id
            ...
            cpu_timer.reset();
    //TODO: we won't use this anymore. we dont use pq distance for navigation
            ...
            for (uint64_t m = 0; m < nnbrs; ++m)
            {
                ...
                if (visited.insert(id).second)//insert successfully
                {
                    if (!use_filter && _dummy_pts.find(id) != _dummy_pts.end())//if it is one of the dummy pts
                        continue;
...
//decode/read data from the sector scratch to node_coords/data_buf, nnbrs --- through offset_to_node, offset_to_node_nhood, offset_to_node_coords
            char *node_disk_buf = offset_to_node(frontier_nhood.second, frontier_nhood.first);//first is the id
            ...
            cpu_timer.reset();
    //navigate -- decide which page to go next
            compute_dists(node_nbrs, nnbrs, dist_scratch);
            if (stats != nullptr)
            {
                stats->n_cmps += (uint32_t)nnbrs;
...
//     }
    ...
    // copy k_search values for "return" the results
    for (uint64_t i = 0; i < k_search; i++)
    {
        indices[i] = full_retset[i].id;
        auto key = (uint32_t)indices[i];
//What do dummy pts do? why use it? what is the dummy to real map?
