...
int build_merged_vamana_index(std::string base_file, diskann::Metric compareMetric, uint32_t L, uint32_t R,
                              ...
                              const uint32_t Lf, const uint32_t num_pq_chunk, const uint32_t page_graph_degree, const uint64_t page_size)
{
...
///MARK: we want the experiment on different R
        ...
        ///TODO: maybe we dont need to save the mem_index_path at all -- we can still access all index info before the index is decontructed
        //_index.save(mem_index_path.c_str());//just save graph data into the mem_index_path 

        std::cout << "Begin to merge close nodes into same pages." << std::endl;
        //std::vector<std::vector<uint32_t>> mergedNodes;
        std::vector<uint32_t> nodeToPageMap;//change it to an array --- arrange by node index map[node_id] = page_ID
        //uint32_t page_graph_degree = 178;
        //uint32_t num_pq_chunk = 10;
        size_t num_pages = _index.mergeNodesIntoPage((uint32_t)base_num, (uint32_t)base_dim, page_graph_degree, R, num_pq_chunk, mergedNodes, nodeToPageMap);//return num of pages
        std::cout << "Number of pages: " << num_pages << std::endl;

        //size_t actual_degree = 32;

        //create disk layout //These are the only file we created
        std::string pq_compressed_all_nodes_path = "pq_compressed_all_nodes.bin";
        std::string final_outputFile = "finalGraphIndex.index";
        _index.create_disk_layout(base_dim, page_graph_degree, pq_compressed_all_nodes_path, final_outputFile, mergedNodes, nodeToPageMap, page_size);
                            ...
                            uint32_t nthreads, uint32_t start_bw)
{
    ...
    while (!stop_flag)
    {   
        std::vector<std::vector<T>> warmupValuesOfReturnNodes;
        std::vector<uint32_t> intersectedVisitedPages;
...
//         {
//             pFlashIndex->cached_beam_search(tuning_sample + (i * tuning_sample_aligned_dim), 1, L, warmupValuesOfReturnNodes, intersectedVisitedPages,
                        ...
                        const std::string reorder_data_file)
{
    ...
    //base reader is to read the base file
                     ...
                     const uint32_t Lf, const uint64_t page_size)
{
    ...
    if (param_list.size() < 5 || param_list.size() > 15)
    {
    ...
    if (param_list.size() < 15) {
        std::cerr << "Error: Expected at least 15 parameters, but got " << param_list.size() << std::endl;
    ...
    const size_t num_pq_chunks = static_cast<size_t>(atoi(param_list[10].c_str()));
    //const uint32_t page_size = (uint32_t)atoi(param_list[11].c_str());
    const float sample_ratio = (float)atof(param_list[12].c_str());//applied
    bool generateLSHBucket = false; //applied
    if (1 == atoi(param_list[13].c_str()))
    {
        generateLSHBucket = true;
    }
    const uint32_t num_nodes_per_lsh_bucket = (uint32_t)atoi(param_list[14].c_str()); //applied

    // std::cout << "Parsed Values:\n";
    // std::cout << "pageGraphDegree: " << page_graph_degree << "\n";
    // std::cout << "num_pq_chunks: " << num_pq_chunks << "\n";
    // std::cout << "page_size: " << page_size << "\n";
    // std::cout << "sample_ratio: " << sample_ratio << "\n";
    // std::cout << "use_lsh: " << (generateLSHBucket ? "true" : "false") << "\n";
    // std::cout << "num_nodes_per_lsh_bucket: " << num_nodes_per_lsh_bucket << "\n";
    // return -1;
    ...
    // }
    //size_t num_pq_chunks = (size_t)(std::floor)(uint64_t(final_index_ram_limit / points_num));//ram limit is in unit of bytes -- this gets average size of points allowed for ram in Bytes

    //size_t num_pq_chunks = 12;
    ...
    //size_t num_pq_chunks = 10;//given we only give pq compressed data space of 12B -- num centers, 256, decide 1 byte each segments. hence, there could be only 12 segments

    std::string pq_pivots_path_tem = "pq_pivots.bin"; //this is to store those pivots/centroids

    std::string pq_compressed_all_nodes_path = "pq_compressed_all_nodes.bin";//this is to store pq compressed of all nodes within a page
    ...
    generate_quantized_data<T>(data_file_to_use, pq_pivots_path_tem, pq_compressed_all_nodes_path, compareMetric, p_val,
                              num_pq_chunks, use_opq, codebook_prefix);

    diskann::cout << timer.elapsed_seconds_for_step("generating quantized data") << std::endl;

    uint32_t num_pq_chunk = static_cast<uint32_t>(num_pq_chunks);
    //uint32_t page_graph_degree = 178;
...
// This code should only be available in the "build" DLL.
    ...
    diskann::build_merged_vamana_index<T, LabelT>(data_file_to_use.c_str(), diskann::Metric::L2, L, R, p_val,
                                                  ...
                                                  labels_to_medoids_path, universal_label, Lf, num_pq_chunk, page_graph_degree, page_size);
    diskann::cout << timer.elapsed_seconds_for_step("building merged vamana index") << std::endl;

    //delete tem files from the filesystem. 
    std::remove(pq_compressed_all_nodes_path.c_str());
    ...
    ////Modificatin: add codes of merge neighbor nodes here
    //1. calculate the num of vectors that each page contains in the new layout
    
    //begin with certain points
    // uint32_t npts, ndims;
    ...
    // size_t write_blk_size = read_blk_size;
    // cached_ifstream base_reader(base_file, read_blk_size);
    // base_reader.read((char *)&npts, sizeof(uint32_t)); //base file contains only nodes' coordinates (namely dim * T); it doesn't contain curr nodeid
    // base_reader.read((char *)&ndims, sizeof(uint32_t));

    // //These are converted to size_t for compatibility with calculations that might require larger ranges
    // size_t npts_64, ndims_64;
    // npts_64 = npts; //this is for compatablity in calculate nnodes_per_sector
    // ndims_64 = ndims;

    // uint32_t width_u32;
    // size_t index_file_size;
    // std::ifstream vamana_reader(mem_index_path, std::ios::binary);
    // vamana_reader.read((char *)&index_file_size, sizeof(uint64_t)); //vamaba reader contains the neigbor info of the each node from 0 to n-1; it doesn't contain curr nodeid
    // vamana_reader.read((char *)&width_u32, sizeof(uint32_t));
    // uint64_t max_node_len, max_nbrs_space, nnodes_per_sector;
    // //UPDATE
    // max_nbrs_space = ((uint64_t)width_u32 + 1) * sizeof(uint32_t); //nbrIDs + num of nbrs
    ...
    // diskann::cout << "disk layouting max_nbrs_space: " << max_nbrs_space << "B" << std::endl;
    // diskann::cout << "disk layouting max_node_len: " << max_node_len << "B" << std::endl;
    // diskann::cout << "disk layouting nnodes_per_sector: " << nnodes_per_sector << "B" << std::endl;

    //_index.mergeNodesIntoPage(data_file_to_use.c_str(), npts, nnodes_per_sector);



//solution 1:
//leftVectors = set containing n integers from 0 to n-1;
//dupCount = 0;
//while(leftVectors.isNotEmpty):
    //i = leftVectors.first
    //newNbrs, dupNbrs = findNN(i, nnodes_per_sector - 1, K, leftVectors) -- return nnodes_per_sector - 1 nearest leftVectors within the K NNs. if no enough nbrs, return dupNbrs
    //assert: newNbrs.size + dupNbrs.size = (nnodes_per_sector - 1):
    //dupCount += dupNbrs.size
    //leftVectors.removeAll(newNbrs)
    //write those group vectors into a page
    //vecBuffer.insert(getVector(i))
    //for nbr in newNbrs:
    //  vecBuffer.insert(getVector(newNbrs))
    //for nbr in dupNbrs
    //  vecBuffer.insert(getVector(dupNbrs))
    //diskann.newWrite(vecBuffer)

//all the data i need: npts, ndims, width_u32, --- these data can be returned/passed as parameter to create_disk_layout
//todo: findNN
//todo:getVector
//todo: diskann.newWrite

//solution 2 merging nbrs: bfs
//leftVectors = a set containing n integers from 0 to n-1;
//dupCount = 0;
//queue<int> q;
//meanDistOfPages = []
//while(leftVectors.isNotEmpty):
    ...
    //while(!q.empty()):
        //int curr = q.pop()
        //leftVectors.remove(curr)
        //assert(nnodes_per_sector -  1 <= _graph[curr].size())
        //nbrs =  _graph[curr].nbrs
        //dist_arr = computeDist(curr, nbrs)
        //id_dist_pairs = []
        //for k in 0 to nbrs.size()-1:
            //id_dist_pairs.insert(<nbrs[k], dist[k]>)
        //id_dist_pairs.sortAscendingBasedOnDist()
        
        //selectedNodes = [curr]
        //selectedNodesDist = []
        //dupCandidateNodes = []
        //dupCandidateNodesDists = []
        //for id, dist in id_dist_pairs:
            //if selectedNodes.size < nnodes_per_sector:
                //if leftVectors.contains(id):
                    //selectedNodes.insert(id)
                    //leftVectors.remove(id)
                    //selectedNodesDist.insert(dist)
                //else:
                    //dupCandidateNodes.insert(id)
                    //dupCandidateNodesDists.insert(dist)
            //else:   //---add the left nodes into queue
                //if leftVectors.contains(id):
                    //queue.add(id)
        //if selectedNodes.size < nnodes_per_sector:
            //selectedNodes.insert(dupCandidateNodes[0:(nnodes_per_sector - selectedNodes.size)])
            //selectedNodesDist.insert(dupCandidateNodesDist[0:(nnodes_per_sector - selectedNodes.size)])
            //dupCount += nnodes_per_sector - selectedNodes.size

        //diskWriter.write(selectedNodes)
        //meanDistOfPages.insert(selectedNodesDist.mean())
...
//     }
    ...
    diskann::cout << "Indexing time: " << diff.count() << std::endl;
     ...
     ///NOTE: usage of the sample points? --- for warming up stage in searching process
     if (sample_ratio != 0){
        double ten_percent_points = std::ceil(points_num * sample_ratio);
    ...
    if (generateLSHBucket){
        ...
        std::ofstream outFilePM("projectionMatrix.bin", std::ios::binary);
        if (!outFilePM) {
        ...
        size_t estimated_elements = 1000000;
        //const size_t num_nodes_per_lsh_bucket = 10;
        tsl::robin_map<uint32_t, uint32_t*> buckets;
        tsl::robin_map<uint32_t, size_t> bucket_sizes;
        buckets.reserve(std::min<size_t>(maxDistinctBuckets, estimated_elements));

        //read data points from final index; or just the first data of each page????
        //read number of total pages first
        std::ifstream reader;
        std::string final_outputFile = "finalGraphIndex.index";
        reader.exceptions(std::ios::badbit | std::ios::failbit);
        reader.open(final_outputFile, std::ios::binary);
        reader.seekg(0, reader.beg);

        uint32_t nr, nc; // metadata itself is stored as bin format (nr is number of metadata, nc should be 1)
        READ_U32(reader, nr);//this is the number of meta info and the dimension of each data info
        READ_U32(reader, nc);

        uint64_t disk_nnodes;
        uint64_t disk_ndims; // can be disk PQ dim if disk_PQ is set to true
        READ_U64(reader, disk_nnodes);
        READ_U64(reader, disk_ndims);
        diskann::cout << "#points: " << disk_nnodes << " #dim: " << disk_ndims << std::endl;

        size_t medoid_id_on_file, max_node_len, nnodes_per_sector, pq_ndims, node_degree;
        READ_U64(reader, medoid_id_on_file);
        READ_U64(reader, max_node_len);
        READ_U64(reader, nnodes_per_sector);
        ///MARK: newly added
        READ_U64(reader, pq_ndims);
        READ_U64(reader, node_degree);

        diskann::cout << "Disk-Index File Meta-data: ";
        diskann::cout << "Medoid's pageID: " << medoid_id_on_file;
        diskann::cout << ", PQ dim: " << pq_ndims;
        diskann::cout << ", # nodes per sector: " << nnodes_per_sector;
        diskann::cout << ", max node len (bytes): " << max_node_len;
        diskann::cout << ", actual page-graph degree: " << node_degree << std::endl;

        size_t num_sector = (disk_nnodes + nnodes_per_sector - 1) / nnodes_per_sector;
        std::cout << num_sector << " pages in total.\n";
        //nnodes_per_sector is what we need
        //the way to organize the data -- all data and nbrID and its PQ value
        std::unique_ptr<T[]> curr_vec_T = std::make_unique<T[]>(nnodes_per_sector * dim);
        std::unique_ptr<float[]> curr_vec_float = std::make_unique<float[]>(nnodes_per_sector * dim);
        //cannot read everything into a buffer, hence, read page by page

//compare only first node within each page
        // for(size_t page_ID = 0; page_ID < num_sector; page_ID++){
        //     if (page_ID % 10000 == 0 && page_ID != 0) {
        //         std::cout << page_ID << " pages processed.\n";
        //     }

        //     ///MARK: read all vectors within the page
        //     reader.seekg((1 + page_ID) * defaults::SECTOR_LEN, reader.beg);
        //     reader.read((char *)(curr_vec_T.get()), sizeof(T) * dim);
        //     diskann::convert_types<T, float>(curr_vec_T.get(), curr_vec_float.get(), 1, dim);

        //     uint32_t hash = 0;
        //     for (int i = 0; i < numProjections; ++i) {
        //         float dotProduct = 0.0;
        //         for (size_t j = 0; j < dimensions; ++j) {
        //             dotProduct += curr_vec_float[j] * projectionMatrix[i][j];
        //         }
        ...
        //     buckets[hash] = static_cast<uint32_t>(page_ID); 
        // }
        
//compare every nodes within each page
        for(size_t page_ID = 0; page_ID < num_sector; page_ID++){
            if (page_ID % 10000 == 0 && page_ID != 0) {
                std::cout << page_ID << " pages processed.\n";
            }
            ...
            ///MARK: read all vectors within the page
            reader.seekg((1 + page_ID) * defaults::SECTOR_LEN, reader.beg);
            reader.read((char *)(curr_vec_T.get()), nnodes_per_sector * sizeof(T) * dim);
            diskann::convert_types<T, float>(curr_vec_T.get(), curr_vec_float.get(), nnodes_per_sector, dim);

            for(size_t k = 0; k < nnodes_per_sector; k++){
            // for(size_t k = 0; k < 1; k++){
                ...
                for (int i = 0; i < numProjections; ++i) {
                    float dotProduct = 0.0;
                    for (size_t j = 0; j < dimensions; ++j) {
                        dotProduct += curr_vec_float[j + k * dim] * projectionMatrix[i][j];
                    }
                    if (dotProduct >= 0) {
                        hash |= (1U << i); // Use (1ULL << i) to create a number where only the i-th bit is 1 //Perform a bitwise OR (|=) with hash to set the bit without affecting other bits
                    }
                ...
                // Only insert if the bucket is empty
                uint32_t node_id = static_cast<uint32_t>(page_ID * nnodes_per_sector + k);
                auto iter = buckets.find(hash);
                if (iter != buckets.end()) {
                    if (bucket_sizes[hash] < num_nodes_per_lsh_bucket) {
                        uint32_t* bucket_data = iter->second;
                        bucket_data[bucket_sizes[hash]++] = node_id;
                    } 
                    // Else: Ignore extra elements to keep max x per bucket
                } else {
                    uint32_t* new_bucket = new uint32_t[num_nodes_per_lsh_bucket];
                    new_bucket[0] = node_id; // Add the first node
        ...
        std::cout << buckets.size() << " buckets in total. Max buckets: " << maxDistinctBuckets << std::endl;

        //write buckets into disk
        std::string bucketsFile = "hypercube_buckets.bin";
        std::ofstream outFileBuckets(bucketsFile, std::ios::binary);
        if (!outFileBuckets) {
        ...
        for (const auto& [hash, idPtr] : buckets) {
            outFileBuckets.write(reinterpret_cast<const char*>(&hash), sizeof(hash));

            // Write the number of elements in the vector
            size_t idCount = bucket_sizes[hash];
            outFileBuckets.write(reinterpret_cast<const char*>(&idCount), sizeof(idCount));

            // Write the vector elements
            outFileBuckets.write(reinterpret_cast<const char*>(idPtr), idCount * sizeof(uint32_t));
        }

        outFileBuckets.close();

        std::cout << "Buckets saved to " << bucketsFile << std::endl;

        // Cleanup: Don't forget to delete dynamically allocated memory when done
...
template DISKANN_DLLEXPORT int build_merged_vamana_index<int8_t, uint32_t>(
    ...
    const std::string &labels_to_medoids_file, const std::string &universal_label, const uint32_t Lf, const uint32_t num_pq_chunk, const uint32_t page_graph_degree, const uint64_t page_size);
