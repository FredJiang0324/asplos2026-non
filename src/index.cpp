...
// template <typename T, typename TagT, typename LabelT>
// size_t Index<T, TagT, LabelT>::mergeNodesIntoPage(const uint32_t npts, const uint32_t ndim, std::string index_prefix_path, const uint32_t page_graph_degree, const uint32_t R, const uint32_t num_pq_chunk, std::vector<std::vector<uint32_t>>& mergedNodes, std::vector<uint32_t>& nodeToPageMap)
// {
//     uint64_t max_node_len, max_nbr_space, nnodes_per_sector, expected_num_pages;
//     max_node_len = (uint64_t)(ndim * sizeof(T));//sizeof will return in unit of Byte
//     max_nbr_space = (uint64_t)(page_graph_degree * (sizeof(uint32_t) + num_pq_chunk * sizeof(uint8_t)));
//     nnodes_per_sector = (uint64_t)((defaults::SECTOR_LEN - max_nbr_space) / max_node_len);
//     expected_num_pages = (npts + nnodes_per_sector - 1) / nnodes_per_sector;
...
//     //modify it.. to group nodes based on their ids
//     tsl::robin_set<uint32_t> unMergedNodesSet;//uint32 can hold 4B different integers
//     for (uint32_t i = 0; i < npts; i++){
//         unMergedNodesSet.insert(i);
//         nodeToPageMap.push_back(-1);
//     }
...
//                 auto distances = computeDist(currNode, candidate_nbrs);//max size: 64-1 or 64*64-1
...
//         int hops = 2; //64^3 -- 4 is not doable -- too much time -- TODO: test at 2?? distance will increase for sure
...
//         while(groupedNodes.size() < nnodes_per_sector || nextNode == -1){
//             fur_hops++;
//             //get frontier
//             tsl::robin_set<uint32_t> new_frontier; //we cannot modify the set while iterating it
...
//             distances = computeDist(currNode, new_candidate_nbrs);//max size: 64-1 or 64*64-1
...
//      ///MARK: this code is for calculatign the mean distance within pages   
...
//         //     }
//         // }
//         // auto distancesToFirstNodeWithinPage = computeDist(currNode, otherNodes);
//         // if (!distancesToFirstNodeWithinPage.empty()) {
//         //     float sumDist = 0.0f;
//         //     for (const auto& dist : distancesToFirstNodeWithinPage) {
//         //         sumDist += dist;
//         //     }
//         //     float meanDist = sumDist / distancesToFirstNodeWithinPage.size();
...
//     std::cout << "Time of beyond 2-hops search and mergy: " << beyond_fixed_hops_search_nbrs << std::endl;
...
// template <typename T, typename TagT, typename LabelT>
// void Index<T, TagT, LabelT>::create_disk_layout(const size_t ndims, const uint32_t page_graph_degree, const std::string &pq_compressed_all_nodes_path, const std::string &output_file, const std::vector<std::vector<uint32_t>>& mergedNodes, const std::vector<uint32_t>& nodeToPageMap, const uint64_t page_size){
...
//     std::unique_ptr<uint8_t[]> pq_compressed_page_all_points = std::make_unique<uint8_t[]>(all_pq_npts * pq_ndims);
//     pqAllPointsReader.read((char*)(pq_compressed_page_all_points.get()), all_pq_npts * pq_ndims * sizeof(uint8_t));
//     pqAllPointsReader.close();

// //step 2: calculate meta data
//     uint64_t max_node_len, max_nbr_space, nnodes_per_sector;
//     max_node_len = (uint64_t)(ndims * sizeof(T));//sizeof will return in unit of Byte
//     max_nbr_space = (uint64_t)(page_graph_degree * (sizeof(uint32_t) + pq_ndims * sizeof(uint8_t)));//32 is the degree of each node
//     nnodes_per_sector = (uint64_t)((defaults::SECTOR_LEN - max_nbr_space) / max_node_len);
...
//     size_t num_node, npts_64, ndims_64;
//     num_node = (uint64_t)nodeToPageMap.size();
//     npts_64 = (uint64_t)(nnodes_per_sector * mergedNodes.size());
//     ndims_64 = (uint64_t)ndims;
// ///MARK: num_node can be different from npts_64
//     diskann::cout << "Number of points in reordered data: " << npts_64 << std::endl;
//     diskann::cout << "Number of dimensions of reordered data: " << ndims << std::endl;
    
//     assert(num_node == _graph_store->getGraphNodesNum());
//     diskann::cout << "Number of nodes in graph: " << num_node << std::endl;
//     //medoid is the _start from vamana index
//     uint64_t medoid = (uint64_t)nodeToPageMap[_start];//_start represents the pageID which contains the node closest to the center
...
//     // uint32_t min_graph_degree = (uint32_t) min;
...
//     output_file_meta.push_back((uint64_t)npts_64);//num of nodes in total in reordered disk -- can be more then node in real graph/dataset
...
//     output_file_meta.push_back((uint64_t)page_graph_degree);///MARK: newly added
...
//             const size_t total_nbrs_to_write = page_graph_degree;//width_u32 * nnodes_per_sector;//should be 32 * 4 = 128
//             tsl::robin_set<uint32_t> alreadyInsertedNodes;
//             alreadyInsertedNodes.reserve(nnodes_per_sector + total_nbrs_to_write);
...
//                 _data_store->get_vector(id, cur_vec);
//                 memcpy(sector_nodes_buf, cur_vec, _data_store->get_aligned_dim() * sizeof(T));
//                 ///NOTE: the nbrID is still the node id instead of pageID, we wanna pageID with the PQ value of the node
//                 allNodesNbrs.push_back(_graph_store->get_neighbours(id));
//                 alreadyInsertedNodes.insert(id);
//                 sector_nodes_buf += max_node_len;
//             }
...
//             uint32_t eachNbrSpace = (uint32_t)(sizeof(uint32_t) + pq_ndims * sizeof(uint8_t));
//             char *sector_nbrs_buf = nullptr;
//             std::vector<uint32_t> iterators(nnodes_per_sector, 0);
//             size_t count = 0;           
//             while(count < total_nbrs_to_write){
...
//                             //writing the nbrPageID and its associated PQ vector values into disk
//                             sector_nbrs_buf = sector_nodes_buf + count * eachNbrSpace;
//                             memcpy(sector_nbrs_buf, &new_reordered_node_ID, sizeof(uint32_t));
//                             //update the sector_nbrs_buf offset
//                             sector_nbrs_buf += sizeof(uint32_t);
...
//                             memcpy(sector_nbrs_buf, &pq_compressed_page_all_points[nbrNodeID * pq_ndims], pq_ndims * sizeof(uint8_t));

//                             //if fill all slots, quit the for loop and while loop
//                             count++;
...
//                     //the value of count not necessary increase at the end of this iteration of for loop
//                     if (count == total_nbrs_to_write){
//                         break;//break the for loop of loop nnodes
//                     }
...
//                 if (count == old_count){
//                     //it must be count < total_nbrs_to_write
// ///TODO: in future, if the nbrs' size is large than or equal to 0xFFFFFFFF, we need to use larger data type like uint64
//                     std::cout << "Not all neighbors spaces are filled up. Only has " << count << " nbrs." << std::endl;
//                     const uint32_t INVALID_NBR_ID = 0xFFFFFFFF;
//                     sector_nbrs_buf = sector_nodes_buf + count * eachNbrSpace;
//                     memcpy(sector_nbrs_buf, &INVALID_NBR_ID, sizeof(uint32_t));
//                     break;//it doesn't fill all -  - the rest node ID and pq data will be 00000
//                 }
...
// template <typename T, typename TagT, typename LabelT>
// uint32_t Index<T, TagT, LabelT>::selectingRAMpageNodes(const size_t ndims, std::vector<std::vector<uint32_t>>& mergedNodes, std::vector<uint32_t>& selectedNodeOriginalIDs, const std::string &ramPageNodeFile, const std::string &ramPageIDFile){
    
//     std::cout << "Begin to sample pages for building routing graph in RAM." << std::endl;
//     std::ofstream sampledPageDataWriter; //(filename, std::ios::binary | std::ios::out);
//     open_file_to_write(sampledPageDataWriter, ramPageNodeFile);
//     std::ofstream selectedPageIdWriter; //(filename, std::ios::binary | std::ios::out);
//     open_file_to_write(selectedPageIdWriter, ramPageIDFile);

//     int ndims_i32 = (int)ndims;
//     size_t nnodes_per_sector = mergedNodes[0].size();
//     size_t num_selected_pages = static_cast<size_t>(std::round(mergedNodes.size() / 12));

//     int num_total_nodes = static_cast<int>(num_selected_pages * nnodes_per_sector);

//     selectedNodeOriginalIDs.reserve(num_total_nodes);

//     sampledPageDataWriter.seekp(0, sampledPageDataWriter.beg);
//     sampledPageDataWriter.write((char *)&num_total_nodes, sizeof(int));
//     sampledPageDataWriter.write((char *)&ndims_i32, sizeof(int));

//     selectedPageIdWriter.seekp(0, selectedPageIdWriter.beg);
//     selectedPageIdWriter.write((char *)&num_selected_pages, sizeof(int));

//     uint32_t selectedPageCount = 0;
//     //std::vector<T> batch_buffer;
//     //batch_buffer.reserve(num_selected_pages * nnodes_per_sector * ndims); // Pre-allocate memory
//     std::unique_ptr<T[]> cur_vec(new T[_data_store->get_aligned_dim()]());

//     double step_size = static_cast<double>(mergedNodes.size()) / num_selected_pages; // floating-point step_size to avoid truncation errors.// Evenly distribute the selections

//     for(size_t i = 0; i < num_selected_pages; i++){
//         uint32_t page_id = static_cast<uint32_t>(std::round(i * step_size));
//         if (page_id >= mergedNodes.size()) break;

//         selectedPageIdWriter.write((char *)&page_id, sizeof(uint32_t));//actually we can calculate this one -- no need to save and read

//         for(size_t j = 0; j < nnodes_per_sector; j++){
//             uint32_t selectedNodeID = mergedNodes[page_id][j];
//             selectedNodeOriginalIDs.push_back(selectedNodeID);
//             _data_store->get_vector(selectedNodeID, cur_vec.get());
//             sampledPageDataWriter.write((char *)cur_vec.get(), ndims * sizeof(T));
//             //Batch append data
//             //batch_buffer.insert(batch_buffer.end(), cur_vec.get(), cur_vec.get() + ndims);
//             //std::memcpy(batch_buffer.data() + (selectedPageCount * nnodes_per_sector * ndims) + (j * ndims), cur_vec.get(), ndims * sizeof(T));//memcpy() does not increase the vector’s size—it only copies bytes into existing memory.
//         }
//         selectedPageCount++;
//     }

//     // // Perform batched write to disk
//     // sampledPageDataWriter.write(reinterpret_cast<char*>(batch_buffer.data()), batch_buffer.size() * sizeof(T));

//     selectedNodeOriginalIDs.shrink_to_fit();

//     num_total_nodes = static_cast<int>(selectedPageCount * nnodes_per_sector);

//     sampledPageDataWriter.seekp(0, sampledPageDataWriter.beg);
//     sampledPageDataWriter.write((char *)&num_total_nodes, sizeof(int));
//     sampledPageDataWriter.close();

//     int selectedPageCenterCount_i32 = (int)selectedPageCount;
//     selectedPageIdWriter.seekp(0, selectedPageIdWriter.beg);
//     selectedPageIdWriter.write((char *)&selectedPageCenterCount_i32, sizeof(int));
//     selectedPageIdWriter.close();
    
//     std::cout << "End of writing selected page Data. Wrote " << selectedPageCount << " selected pages into disk" << std::endl;
//     return static_cast<uint32_t>(num_total_nodes);
// }

// template <typename T, typename TagT, typename LabelT>
// void Index<T, TagT, LabelT>::write_topo_ram_page_nodes_in_disk(const uint32_t num_ram_page_nodes, const std::string &ramPageNodeTopoFile){
//     std::cout << "Begin to write selected page nodes' topology into disk" << std::endl;
//     std::ofstream topoWriter; //(filename, std::ios::binary | std::ios::out);
//     open_file_to_write(topoWriter, ramPageNodeTopoFile);

//     uint32_t degree = 32;
//     //topoWriter.seekp(0, topoWriter.beg);
//     topoWriter.write((char *)&num_ram_page_nodes, sizeof(uint32_t));
//     topoWriter.write((char *)&degree, sizeof(uint32_t));
//     //write medoid
//     uint32_t medoid_index = _start;
//     topoWriter.write((char *)&medoid_index, sizeof(uint32_t));

//     for(uint32_t i = 0; i < num_ram_page_nodes; i++){
//         size_t count = 0;
//         for(const auto& nbrID : _graph_store->get_neighbours(i)){
//             topoWriter.write((char *)&nbrID, sizeof(uint32_t));
//             count++;
//         }

//         if (count < degree){
//             std::cout << "Not enough neighbors for " << i << "th node" <<std::endl;
//         }

//         while (count < degree) {
//             uint32_t padding = UINT32_MAX;
//             topoWriter.write(reinterpret_cast<const char*>(&padding), sizeof(uint32_t));
//             count++;
//         }
//     }
//     topoWriter.close();
//     std::cout << "Finished writing topology for " << num_ram_page_nodes << " nodes." << std::endl;
// }

// template <typename T, typename TagT, typename LabelT>
// void Index<T, TagT, LabelT>::write_pq_selected_ram_page_nodes(const std::string &pq_compressed_all_nodes_path, const std::string &pq_compressed_selected_nodes_path, const std::vector<uint32_t>& selectedNodeOriginalIDs){
...
//     std::cout << "Begin to write selected page nodes' pq values into disk" << std::endl;
//     std::ofstream pqValueWriter; //(filename, std::ios::binary | std::ios::out);
//     open_file_to_write(pqValueWriter, pq_compressed_selected_nodes_path);

//     uint32_t num_selected_nodes = static_cast<uint32_t>(selectedNodeOriginalIDs.size());
//     pqValueWriter.write((char *)&num_selected_nodes, sizeof(uint32_t));
//     pqValueWriter.write((char *)&pq_ndims, sizeof(int));

//     //based on the nodeID to retrive their pq --- modify the selectingRAMpageNodes
//     for (const auto& id: selectedNodeOriginalIDs){
//          pqValueWriter.write((char *)(&pq_compressed_page_all_points[id * pq_ndims]), pq_ndims * sizeof(uint8_t));
//     }

//     pqValueWriter.close();
//     std::cout << "Finished writing selected page nodes' pq values into disk" << std::endl;
