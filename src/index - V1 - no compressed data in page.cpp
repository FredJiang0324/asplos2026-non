...
void Index<T, TagT, LabelT>::build(const T *data, const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    std::cout << "Build with input of data in mem begins" << std::endl;
...
template <typename T, typename TagT, typename LabelT>
std::vector<float> Index<T, TagT, LabelT>::computeDist(uint32_t node, const std::vector<location_t>& neighbors) {
    std::vector<float> distances;
    for (const auto& neighbor : neighbors) {
        distances.push_back(_data_store->get_distance(node, neighbor));
    }
    return distances;
}

template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::mergeNodesIntoPage(const uint32_t npts, const uint32_t ndim, const uint64_t nnodes_per_sector, const std::string &tem_file, std::vector<T> &pageVectors)
{
    //do we need to load _index._graph_store?? probably not. i didn't see any place clear it
    bool test = false;

    if (test){
        //this is the distances for default layout -- we use it for baseline
        std::vector<float> mergeMeanDistances;
        for (uint32_t i = 0; i < npts;){

            std::vector<uint32_t> groupedNodes;
            for (uint32_t k = 0; k < nnodes_per_sector;k++){
                if (i + k < npts) {
                    groupedNodes.push_back(i + k);
                }
            }

            std::vector<float> meanDistances;
            for (const auto& currNode : groupedNodes){
                std::vector<uint32_t> otherNodes;
                for (const auto& node : groupedNodes) {
                    if (node != currNode) {
                        otherNodes.push_back(node);
                    }
                }
                auto distances = computeDist(currNode, otherNodes);
                if (!distances.empty()) {
                    float sumDist = 0.0f;
                    for (const auto& dist : distances) {
                        sumDist += dist;
                    }
                    float meanDist = sumDist / distances.size();
                    meanDistances.push_back(meanDist);
                }
            }
            
            if (!meanDistances.empty()) {
                float sumDist = 0.0f;
                for (const auto& dist : meanDistances) {
                    sumDist += dist;
                }
                float meanDist = sumDist / meanDistances.size();
                mergeMeanDistances.push_back(meanDist);
            }

            i += (uint32_t)nnodes_per_sector;

        }

        std::cout << "Pages Count: " << mergeMeanDistances.size() << std::endl;

        std::ofstream distWriter("distances_baseline.txt");

        //std::cout << "Mean Distances of Pages:" << std::endl;
        for (const auto& dist : mergeMeanDistances) {
            distWriter << dist << std::endl;
        }

        distWriter.close();

    }else{
        tsl::robin_set<uint32_t> unprocessedNodes;//uint32 can hold 4B different integers
        for (uint32_t i = 0; i < npts; i++){
            unprocessedNodes.insert(i);
        }
        ...
        while (!unprocessedNodes.empty()) {
            ...
            uint32_t currNode = *it;
            unprocessedNodes.erase(currNode);
            //std::cout << "Left nodes: " << unprocessedNodes.size() << std::endl;
            int hops = 1; //4 is not doable -- too much time
            //make a set of nbrs
            tsl::robin_set<uint32_t> nbrs_set;
            tsl::robin_set<uint32_t> visited_nbrs_set;
            nbrs_set.insert(currNode);
            for (int hop = 0; hop < hops; ++hop) {
                tsl::robin_set<uint32_t> new_nbrs_set; //we cannot modify the set while iterating it

                for (const auto& n : nbrs_set){
                    if (visited_nbrs_set.insert(n).second){//inserting successfully means it is not visited before
                ...
                nbrs_set = std::move(new_nbrs_set);
            }

            //then cover the nbrs from set to vectors
            std::vector<uint32_t> nbrs(nbrs_set.begin(), nbrs_set.end());

            auto distances = computeDist(currNode, nbrs);//max size: 64-1 or 64*64-1

            std::vector<std::pair<uint32_t, float>> id_dist_pairs;
            for (uint32_t i = 0; i < nbrs.size(); ++i) {
                id_dist_pairs.emplace_back(nbrs[i], distances[i]);
            }

            std::sort(id_dist_pairs.begin(), id_dist_pairs.end(), 
                    [](const auto& a, const auto& b) { return a.second < b.second; });

            std::vector<uint32_t> groupedNodes = {currNode};
            std::vector<uint32_t> dupCandidateNodes;
            
            for (const auto& id_dist : id_dist_pairs) {
                uint32_t id = id_dist.first; //maybe uint32_t ?
                float dist = id_dist.second;

                if (groupedNodes.size() < nnodes_per_sector) {
                    if (unprocessedNodes.find(id) != unprocessedNodes.end()) {
                        groupedNodes.push_back(id);
                        unprocessedNodes.erase(id);
                    } else {
                        dupCandidateNodes.push_back(id);
                    }
                }
            }

            if (groupedNodes.size() < nnodes_per_sector) {
                //this is necessary since all nbrs may be less than nnodes_per_sector
                size_t toAdd;
                if (nnodes_per_sector - groupedNodes.size() <= static_cast<int>(dupCandidateNodes.size())){
                    toAdd = nnodes_per_sector - groupedNodes.size();
                }else{
                    toAdd = static_cast<int>(dupCandidateNodes.size());
                    std::cout << "Nnbrs is smaller the nnodes_per_sector" << std::endl;
                }
                groupedNodes.insert(groupedNodes.end(), dupCandidateNodes.begin(), dupCandidateNodes.begin() + toAdd);
                dupCount += toAdd;
            }
            mergedNodes.push_back(groupedNodes);
            
            //calculate the mean distance to each node within this page
            std::vector<float> meanDistances;
            for (const auto& currNode : groupedNodes){
                std::vector<uint32_t> otherNodes;
                for (const auto& node : groupedNodes) {
            ...
            //average the mean distances within this page
            if (!meanDistances.empty()) {
                float sumDist = 0.0f;
                for (const auto& dist : meanDistances) {
                    sumDist += dist;
            ...
            //std::cout << "After processing, left nodes: " << unprocessedNodes.size() << std::endl;

        }//end of while loop

        std::cout << "Pages Count: " << mergedNodes.size() << std::endl;

        std::ofstream distWriter("distances.txt");
        ...
        std::cout << "Duplicate Count: " << dupCount << std::endl;

        std::ofstream writer; //(filename, std::ios::binary | std::ios::out);
        open_file_to_write(writer, tem_file);
        int npts_i32 = (int)npts, ndims_i32 = (int)ndim;//this is to be compatible with load_bin_impl function in utils.h
        //size_t bytes_written = 2 * sizeof(uint32_t) + npts * ndims * sizeof(T);

        std::cout << "Begin to write temReorder Data" << std::endl;
        writer.seekp(0, writer.beg);
        ...
        // }
        uint32_t count = 0;
        T *cur_vec = new T[_data_store->get_aligned_dim()];
        std::memset(cur_vec, 0, _data_store->get_aligned_dim() * sizeof(T));
        for (const auto& inner_vector : mergedNodes) {
            // Loop through each element in the current inner vector
            for (const auto& i : inner_vector) {
                // the std::ofstream::write function expects a pointer to a character type (char*) as its first argument.
                // It expects a pointer to char (or const char) because char is the fundamental unit for representing bytes in C++
                //Without the cast, the type of data + i * aligned_dim would be T*.
                //By casting to char*, you are telling the compiler to treat the memory starting at data + i * aligned_dim as a sequence of bytes.
                //_data_store->get_vector(i, (T *)cur_vec);
                _data_store->get_vector(i, cur_vec);
                writer.write((char *)cur_vec, ndim * sizeof(T));//in our case ndim == _aligned_dim
                if (count % nnodes_per_sector == 0) {
                    pageVectors.insert(pageVectors.end(), cur_vec, cur_vec + ndim);
                }
                ++count;
            }
        }
        int count_i32 = (int)count;
        writer.seekp(0, writer.beg);
        writer.write((char *)&count_i32, sizeof(int));

        delete[] cur_vec;
        writer.close();

        std::cout << "End of writing temReorder Data. Wrote " << count << " nodes into disk" << std::endl;

        //memcpy(dest, _data + i * _aligned_dim, this->_dim * sizeof(data_t));

        //_data + locations[i] * _aligned_dim
        //float *center = new float[_aligned_dim];
        //center[j] += (float)_data[i * _aligned_dim + j];

        //const data_t *cur_vec = _data + (i * (size_t)_aligned_dim); //data_t will determine cur_vec will be the first vector
...
template <typename T, typename TagT, typename LabelT>
void Index<T, TagT, LabelT>::create_disk_layout(const size_t num_page, const std::string &reOrderedData_file, const std::string &output_file){
    uint32_t npts, ndims;

    std::ifstream baseReader;
    baseReader.exceptions(std::ios::badbit | std::ios::failbit);
    baseReader.open(reOrderedData_file, std::ios::binary);
    baseReader.seekg(0, baseReader.beg);

    int npts_i32, dim_i32;//the first two elements 
    //std::ifstream::read function expects a pointer to a char (or unsigned char or std::byte) for its first argument. 
    //This is because read is designed to read raw bytes from a file into memory. 
    //The cast to (char *) ensures that the pointer to the data is in the correct format for the read function to process.
    baseReader.read((char *)&npts_i32, sizeof(int));
    baseReader.read((char *)&dim_i32, sizeof(int));
    npts = (uint32_t)npts_i32;
    ndims = (uint32_t)dim_i32;

    // amount to read or write in one shot
    size_t write_blk_size = 64 * 1024 * 1024;

    diskann::cout << "Number of points in reordered data: " << npts << std::endl;
    assert(num_page == _graph_store->getGraphNodesNum());
    diskann::cout << "Number of nodes in graph: " << num_page << std::endl;
//may need to modify
    ...
    uint32_t width_u32 = (uint32_t) max;
    diskann::cout << "Max degree of page graph: " << width_u32 << std::endl;
    diskann::cout << "Min degree of page graph: " << min << std::endl;

//we have _graph, we dont need vamana_reader ---- note: the index of _graph is the pageNum/pageIndex
    //std::ifstream vamana_reader(mem_index_file, std::ios::binary);
    cached_ofstream diskann_writer(output_file, write_blk_size);

    //recompute -- including closest vector index
    uint64_t max_node_len, nnodes_per_sector, max_nbrs_space, max_half_nnodes_per_sector, each_nbr_space;

    //uint8_t -- one byte;
    //sizeof will return in unit of Byte
    max_node_len = (uint64_t)(ndims_64 * sizeof(T) + sizeof(uint8_t));// dataValue + index of closestPage
    max_half_nnodes_per_sector = (defaults::SECTOR_LEN / max_node_len) / 2; //temporarily
    each_nbr_space = (uint64_t)(sizeof(uint32_t) + max_half_nnodes_per_sector * sizeof(uint8_t));//nbrPageID (4 B each) + top 3 closestVectorIndexOfEachNbrPages (1 B each)
    max_nbrs_space = (uint64_t)(sizeof(uint8_t) + width_u32 * each_nbr_space); //num of nbrPages (1 B since it is less than 255) + nbrPagesInfo
    
    //SECTOR_LEN == 4096 B = 4KB
    nnodes_per_sector = (defaults::SECTOR_LEN - max_nbrs_space) / max_node_len; // 0 if max_node_len > SECTOR_LEN
    diskann::cout << "Section Length: " << defaults::SECTOR_LEN << "B" << std::endl;
    diskann::cout << "max_nbrs_space: " << max_nbrs_space << "B" << std::endl;
    diskann::cout << "max_node_len: " << max_node_len << "B" << std::endl;
    diskann::cout << "nnodes_per_sector: " << nnodes_per_sector << std::endl;
    diskann::cout << "Each neighbor page records its top " << max_half_nnodes_per_sector << " closest vectors' indices" <<std::endl;

    // defaults::SECTOR_LEN buffer for each sector
    std::unique_ptr<char[]> sector_buf = std::make_unique<char[]>(defaults::SECTOR_LEN);
    //std::unique_ptr<char[]> multisector_buf = std::make_unique<char[]>(ROUND_UP(max_node_len, defaults::SECTOR_LEN));//this is used for the case where a vector cannot be stored within a single sector
    std::unique_ptr<char[]> node_buf = std::make_unique<char[]>(max_node_len);
    //std::unique_ptr<char[]> nbr_page_buf = std::make_unique<char[]>(each_nbr_space);
    //modified correctly: defines a reference to an uint8_t located at a specific offset, after all nodes data
    //uint8_t &nnbrs = *(uint8_t *)(node_buf.get() + nnodes_per_sector * (ndims_64 * sizeof(T) + sizeof(uint8_t)));
    //modified correctly: defines a pointer to an uint32_t located at a specific offset in the node_buf.
    //uint32_t *nhood_buf = (uint32_t *)(node_buf.get() + nnodes_per_sector * (ndims_64 * sizeof(T) + sizeof(uint8_t)) + sizeof(uint8_t));

    // number of sectors (1 for meta data)
    //rounds up the number of points (npts_64) divided by the number of nodes per sector. 
    ...
    uint64_t disk_index_file_size = (n_sectors + 1) * defaults::SECTOR_LEN; //+1 for metadata
    
    std::vector<uint64_t> output_file_meta;
    output_file_meta.push_back(npts_64);//num of nodes in total in reordered disk
    output_file_meta.push_back(ndims_64);
    //medoid is the _start from vamana index
    uint64_t medoid = (uint64_t)_start;//_start represents the pageID closest to the center
...
///MARK: with frozen points --- we use the default value which is 0
...
//This is common in file systems and database files where specific headers or metadata are required at the beginning of the file.
    ...
    assert(n_sectors == num_page);
    diskann::cout << "calculated # sectors is matched with the # of generated pages in graph"<< std::endl;
    uint64_t cur_node_id = 0;
//Done: reviewed one more time
    if (nnodes_per_sector > 0)
    { // Write multiple nodes per sector
        for (uint32_t sector = 0; sector < n_sectors; sector++)
        {
            if (sector % 10000 == 0)
            {
            ...
            std::vector<std::vector<T>> nodesWithinPage;
            //dynamic pointer to an array of T, which will be destroyed auto when out of scope
            std::unique_ptr<T[]> cur_node_coords = std::make_unique<T[]>(ndims_64);//The argument ndims_64 specifies the size of the array. 

            for (uint64_t sector_node_id = 0; sector_node_id < nnodes_per_sector && cur_node_id < npts_64;
                 sector_node_id++)
            {   //.get() is used with smart pointer
                //Use .get() when you need the raw pointer from a smart pointer for operations that require a raw pointer
                baseReader.read((char*)(cur_node_coords.get()), sizeof(T) * ndims_64);
                //adding 1 to a pointer does not increase the pointer value by 1 byte, but by sizeof(T) bytes
                nodesWithinPage.push_back(std::vector<T>(cur_node_coords.get(), cur_node_coords.get() + ndims_64));
                cur_node_id++;
            }
            
            //IDS of the nbrs of this page//sector index is the page's ID
            std::vector<uint32_t> pageNbrs = _graph_store->get_neighbours(sector);
            uint8_t numPageNbrs = (uint8_t)pageNbrs.size();
            //get pageNbrs's representative values -- 64 Values
            std::vector<std::vector<T>> valuesOfNbrPages;
            std::unique_ptr<T[]> vec = std::make_unique<T[]>(ndims_64);
            for (const auto& nbr : pageNbrs){
                _data_store->get_vector(nbr, vec.get());
                //The constructor std::vector<T>(start, end) creates a vector containing elements in the range [start, end).
                valuesOfNbrPages.push_back(std::vector<T>(vec.get(), vec.get() + ndims_64));//
            }

            //MARK: get the index of the page cloest to each nodes within the page
            std::vector<uint8_t> closestPageOfEachNode;

            for (const auto& node : nodesWithinPage){
                uint8_t closestPageIndex = 0;
                T minDist = std::numeric_limits<T>::max();
                //iterate each neighbor page
                for (uint8_t i = 0; i < valuesOfNbrPages.size(); ++i) {
                    T dist = 0;
                    //acculmulate each dimension
                    for (size_t j = 0; j < ndims_64; j++){
                        T diff = node[j] - valuesOfNbrPages[i][j];
                        dist += diff * diff;
                    }
                ...
                closestPageOfEachNode.push_back(closestPageIndex);
            }

            //MARK: get the indices of top k cloest vectors to each neighbor page
            std::vector<std::vector<uint8_t>> closestVectorsOfNbrPages(numPageNbrs, std::vector<uint8_t>(max_half_nnodes_per_sector));
            for (size_t nbrPageIndex = 0; nbrPageIndex < valuesOfNbrPages.size(); ++nbrPageIndex){
                const auto& nbrPageValue = valuesOfNbrPages[nbrPageIndex];

                std::vector<std::pair<uint8_t, T>> id_dist_pairs;
                //iterate each vector
                for (uint8_t i = 0; i < nodesWithinPage.size(); ++i) {
                    T dist = 0;
                    //acculmulate each dimension
                    for (size_t j = 0; j < ndims_64; j++){
                        T diff = nbrPageValue[j] - nodesWithinPage[i][j];
                        dist += diff * diff;
                    }
                    id_dist_pairs.emplace_back(i, dist);
                }

                std::sort(id_dist_pairs.begin(), id_dist_pairs.end(), 
                        [](const auto& a, const auto& b) { return a.second < b.second; });

                //MARK: get the top max_half_nnodes_per_sector ids and insert them in order into the closestVectorsOfNbrPages in the corresponding index
                for (size_t k = 0; k < max_half_nnodes_per_sector; ++k) {
                    closestVectorsOfNbrPages[nbrPageIndex][k] = id_dist_pairs[k].first;
                }
            }

//by far we get all needed info -- nodesWithinPage, closestPageOfEachNode, pageNbrs, valuesOfNbrPages, closestVectorsOfNbrPages

//MARK: Store each node's value and their cloest nbrPage's index in node buf correctly one by one
            char *sector_node_buf;
            for (uint8_t i = 0; i < nnodes_per_sector; ++i) {
                memset(node_buf.get(), 0, max_node_len);
                //write coords of node first //.data() is used with containers like std::vector and std::array to get a pointer to the underlying data array.
                memcpy(node_buf.get(), nodesWithinPage[i].data(), ndims_64 * sizeof(T));
                //write the index of its closest page which is uint8
                *(uint8_t *)(node_buf.get() + ndims_64 * sizeof(T)) = closestPageOfEachNode[i];
                // get offset into sector_buf
                sector_node_buf = sector_buf.get() + (i * max_node_len);
                // copy node buf into sector_node_buf
                memcpy(sector_node_buf, node_buf.get(), max_node_len);
            }
            //update the sector_node_buf offset
            sector_node_buf = sector_buf.get() + (nnodes_per_sector * max_node_len);
            //write number of page neighbors next
            *(uint8_t *)sector_node_buf = numPageNbrs;
            
            //write page nhood info next
            for (uint8_t i = 0; i < numPageNbrs; ++i){
                ...
                //write id of nbr page first
                //the type of pageNbrs is uint32_t //& give the address, or you can use pointer of pageNbrs[i] if you have
                memcpy(sector_node_buf, &pageNbrs[i], sizeof(uint32_t));
