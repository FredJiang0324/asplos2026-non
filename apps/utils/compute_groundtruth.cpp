...
int aux_main(const std::string &base_file, const std::string &query_file, const std::string &gt_file, size_t k,
             const diskann::Metric &metric, const std::string &index_prefix,  const std::string &origin_gt_file = std::string(""))
{
    size_t npoints, nqueries, dim;
    float *query_data;

    const bool skip_knn = origin_gt_file.empty() ? false : true;
    ...
    if (skip_knn & tags_enabled){
        //load data from origin_gt_file into original_closest_points and dist_closest_points
        uint32_t *original_closest_points = nullptr;
        size_t gt_num, gt_dim;
        diskann::load_truthset(origin_gt_file, original_closest_points, dist_closest_points, gt_num, gt_dim);
        ...
        for (size_t i = 0; i < nqueries; i++)
        {
            ...
            if (i == 0){
                std::cout << "Top " << 10 << " results for query 0:\n";
                std::cout << "Original closest points (indices): ";
                for (size_t j = 0; j < 10; j++) {
                    std::cout << original_closest_points[j] << " ";
                }
                std::cout << std::endl;

                std::cout << "Mapped closest points (tags): ";
                for (size_t j = 0; j < 10; j++) {
                    std::cout << location_to_tag[original_closest_points[j]] << " ";
                }
                std::cout << std::endl;
            }
        }
...
{
    std::string data_type, dist_fn, base_file, query_file, gt_file, index_prefix, origin_gt_file;
    uint64_t K;
    ...
    try
    {
        ...
        desc.add_options()("origin_gt_file", po::value<std::string>(&origin_gt_file)->default_value(std::string()),
    ...
    try
    {
        if (data_type == std::string("float"))
            aux_main<float>(base_file, query_file, gt_file, K, metric, index_prefix, origin_gt_file);
        if (data_type == std::string("int8"))
            aux_main<int8_t>(base_file, query_file, gt_file, K, metric, index_prefix, origin_gt_file);
        if (data_type == std::string("uint8"))
            aux_main<uint8_t>(base_file, query_file, gt_file, K, metric, index_prefix, origin_gt_file);
    }
