...
template <typename data_t> class InMemOOCDataStore : public AbstractDataStore<data_t>
{
  ...
  private:
    ...
    ///MARK: newly added
    //tsl::robin_map<uint32_t, uint32_t> _nodeID_idx_map; //size of M -- need to shrink_to_fit

    // uint32_t _idx_slot_to_discard = 0;

    // uint32_t _shrink_counter = 0;

    // std::vector<uint32_t> _cached_nodeIDs; //size of this->capacity

    // mutable std::ifstream _base_reader;
