...
//newly added for mergeNodesIntoPages
    //DISKANN_DLLEXPORT size_t mergeNodesIntoPage(const uint32_t npts, const uint32_t ndim, std::string index_prefix_path, const uint32_t page_graph_degree, const uint32_t R, const uint32_t num_pq_chunk, std::vector<std::vector<uint32_t>>& mergedNodes, std::vector<uint32_t>& nodeToPageMap);
    
    //DISKANN_DLLEXPORT uint32_t selectingRAMpageNodes(const size_t ndims, std::vector<std::vector<uint32_t>>& mergedNodes, std::vector<uint32_t>& selectedNodeOriginalIDs, const std::string &ramPageNodeFile, const std::string &ramPageNodeIDFile);

    //DISKANN_DLLEXPORT void write_topo_ram_page_nodes_in_disk(const uint32_t num_ram_page_nodes, const std::string &ramPageNodeTopoFile);

    //DISKANN_DLLEXPORT void write_pq_selected_ram_page_nodes(const std::string &pq_compressed_all_nodes_path, const std::string &pq_compressed_selected_nodes_path, const std::vector<uint32_t>& selectedNodeOriginalIDs);
    
    //DISKANN_DLLEXPORT void create_disk_layout(const size_t ndims, const uint32_t page_graph_degree, const std::string &pq_compressed_all_nodes_path, const std::string &output_file, const std::vector<std::vector<uint32_t>>& mergedNodes, const std::vector<uint32_t>& nodeToPageMap, const uint64_t page_size);
